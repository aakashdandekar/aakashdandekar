...
![ETL Workflows](https://img.shields.io/badge/ETL%20Workflows-6366F1?style=flat&logoColor=white)
...
`Agent` `CLI`
A local CLI-based autonomous agent powered by Ollama for running LLM tasks directly from the terminal.
`Python` `Ollama` `LangChain` `CLI`

### 🎙️ Speech-to-Text for Wayland
`Voice AI` `Linux`
A voice AI integration for the Wayland display server, enabling speech-to-text capabilities on Linux.
...
`Backend` `API`
A backend API for a freelance marketplace platform with secure auth and data validation.
`FastAPI` `MongoDB` `Pydantic` `JWT`
